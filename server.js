...
// ========== INICIALIZAÇÃO DO FIREBASE (Usa variáveis de ambiente) ==========
try {
  ...
  // Caso o usuário use as variáveis separadas (mais comum)
  try {
    admin.initializeApp({
    ...
    console.log('🔥 Firebase conectado (via variáveis separadas)!');
  } catch (e2) {
...
app.post('/api/signup', async (req, res) => {
  ...
  try {
    ...
    // 🔥 REMOVIDO: await admin.auth().createUser(...) 
    // O front-end já criou o usuário no Firebase Auth. Aqui o backend APENAS salva no Firestore.

    const userData = { 
      nome, email, estilos, avatar: '🎸', criadoEm: new Date(), 
      theme: 'dark', fontSize: 16, colorblind: false, discordWebhook: null 
    };
    ...
    console.error('Erro no signup backend:', e.message);
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (!userDoc.exists) return res.status(401).json({ error: 'Usuário não encontrado no banco' });
    ...
    console.error('Erro no login backend:', e.message);
    res.status(500).json({ error: 'Erro ao buscar dados: ' + e.message }); 
  }
});
...
app.post('/api/update-settings', async (req, res) => {
  ...
  await setUserInFirestore(email, settings);
  res.json({ success: true });
});
...
server.listen(PORT, () => console.log(`🎧 Sonora Fan → http://localhost:${PORT}`));const express = require('express');
const http = require('http');
const https = require('https');
const { Server } = require('socket.io');
const path = require('path');
